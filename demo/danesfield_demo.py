...
def run(args):
	parser = argparse.ArgumentParser(description="Download WV3 images to run the Danesfield pipeline")
	parser.add_argument('--site', help='CORE3D site to download images for', \
						default='Jacksonville')
	parser.add_argument('--img_path', help='Location where WV3 images should be downloaded to', \
						default=f'{os.getcwd()}/imgpath')
	parser.add_argument('--run', help='Run Danesfield after downloading images', \
						action='store_true')
	parser.add_argument('--out_path', help='Location that Danesfield should use as an output directory', default='outdir')
	parser.add_argument('--config_dir', help='Location where Danesfield configuration files can be found. \
											You must use input_<region>.ini and <region>_config.json for \
											Danesfield and Vissat config names, respectively.', default='configdir')
	parser.add_argument('--show_sites', help='List available CORE3D sites and exit', \
						action='store_true')
	parser.add_argument('--model_path', help='Location of necessary model files for Danesfield', default="Models")
	parser.add_argument('--key_id', help='AWS key ID')
	parser.add_argument('--secret_key', help='AWS secret key')
	parser.add_argument('--pull_image', help='Pull Danesfield image from Kitware\'s repository', \
						action='store_true')
	parser.add_argument('--pull_models', help='Pull models from data.kitware.com', action='store_true')
	args = parser.parse_args(args)

	dset = 'spacenet-dataset'
	bucket = 'Hosted-Datasets/CORE3D-Public-Data/Satellite-Images/'
	client = boto3.client('s3', aws_access_key_id=args.key_id, aws_secret_access_key=args.secret_key)
	...
	if args.pull_models and not os.path.exists(args.model_path):
		...
		cmd_args = ['wget', '-O', f'./models.zip', 'https://data.kitware.com/api/v1/folder/5fa1b5e150a41e3d192de52b/download']
		subprocess.run(cmd_args)
	...
	if args.show_sites:
		print("Sites with available WV3 imagery:", wv3.keys())
		return 0

	# User selected invalid region
	if args.site not in wv3:
		print("Invalid site selected, please choose from amongst the following:", wv3.keys())
		return 1

	# download files for requested region
	for f in wv3[args.site]:
		os.makedirs(os.path.join(args.img_path, '/'.join(f.split('/')[3:-1])), exist_ok=True)
		download_path = os.path.join(args.img_path, '/'.join(f.split('/')[3:]))
		if not os.path.exists(download_path):
			print(f'Downloading: {f}')
			client.download_file(dset, f, os.path.join(args.img_path, '/'.join(f.split('/')[3:])))
	...
	if args.run:
		...
		config['paths']['p3d_fpath'] = os.path.join('/workdir', args.site, 'cloud.las')
		config['paths']['work_dir'] = os.path.join('/workdir', args.site)
		config['paths']['imagery_dir'] = os.path.join('/mnt', args.site, 'WV3')
		config['paths']['aoi_config'] = os.path.join('/configs', args.site+'_config.json')
		...
		config['material']['model_fpath'] = '/models/Rutgers\\ Material\\ Segmentation\\ Model/model_D4_20.pth.tar'
		...
		cmd_args = ['docker', 'run', '-it', '--gpus', 'all', '--shm-size', '8G', \
					'-v', img_path+':/mnt', '-v', out_path+':/workdir', \
					'-v', config_dir+':/configs', '-v', model_path+':/models', \
					'kitware/danesfield', 'source /opt/conda/etc/profile.d/conda.sh && \
					conda activate core3d && python danesfield/tools/run_danesfield.py \
					--image /configs/input_'+args.site+'.ini']
		print(' '.join(cmd_args))
		subprocess.run(cmd_args)
