        ...
        for i in self.geon_type.keys():
            ...
            for si in geon_info['geon'].keys():
                if geon_info['geon'][si] == 'Flat':
                    surf = building.flatsurface[si]
                    surf_cor = surf[0]
                    poly_area = Polygon(surf_cor).area
                    mbr_area = mbr(surf_cor[:, 0:2]).area
                    if (mbr_area - poly_area) / mbr_area >= 0.35:
                        geon_parameter.append(add_mesh_geon(id,
                                                           building.topsurface[si].point_cor,
                                                           building.bottomsurface[si].point_cor,
                                                           model_offset))
                    else:
                        geon_parameter.append(add_box_geon(id,
                                                           surf,
                                                           model_offset))
                    ...
                    geon_parameter.append(add_gable_geon(id,
                                                         top_surfs,
                                                         body_Z,
                                                         model_offset))
                    ...
                    if (mbr_area - poly_area) / mbr_area >= 0.35:
                        ...
                        geon_parameter.append(add_shed_geon(id,
                                                            top_surf,
                                                            body_Z,
                                                            model_offset))
                    else:
