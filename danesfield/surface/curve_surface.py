    ...
    def get_obj_string(self, offset):
        ...
        for i in range(0, self.body_num):
            ...
            for l in self.top_curved_surface_index[i]:
                ...
                bottom_surface_index_str.append('f ' + ' '.join([str(si + point_flag + pn) for si in l.tolist()]) + "\n")
                ...
                for i2 in range(i1, self.top_curved_surface_index[i].shape[0]):
                    if len(list_intersect(self.top_curved_surface_index[i][i1],
                        ...
                        intersect_line.append(
                            list_intersect(self.top_curved_surface_index[i][i1], self.top_curved_surface_index[i][i2]))
            ...
            for j in range(0, len(self.top_curved_surface_index[i])):
                ...
                for k in range(0, surface_pn):
                    flag = 0
                    if k == surface_pn - 1:
                        temp_wall_str = [curved_surface_index[j][k] - point_flag, curved_surface_index[j][0] - point_flag]
                        ...
                        if flag == 0:
                            ...
                            wall_str.append('f ' + ' '.join([str(l) for l in temp_wall_index]) + '\n')
                    else:
                        temp_wall_str = [curved_surface_index[j][k] - point_flag, curved_surface_index[j][k + 1] - point_flag]
                        for l in intersect_line:
                            if len(list_intersect(temp_wall_str, l)) == 2:
                                flag = 1
                                break
