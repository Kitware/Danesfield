...
def add_box_geon(id, surf, offset):
    '''
    Get box geon dictionary
    :param id: geon ID
    :param surf: geon surface
    :param offset:
    :return: box geon dictionary
    '''
    surf_cor = surf[0]
    if surf[1]:
        surf_cor2d = surf[0] + np.array(surf[3])
        surf_cor2d = surf_cor2d[:, 0: 2]
        surf_mbb = mbr(surf_cor2d)
        surf_mbb_rm = np.array([[np.cos(surf_mbb.unit_vector_angle), -np.sin(surf_mbb.unit_vector_angle), 0],
                                ...
                                [0, 0, 1]])
        surf_r = np.array([[1, 0, 0],
                           [0, 1, 0],
                           [0, 0, 1]])
        affine_m = np.dot(surf_r, surf_mbb_rm)
        center = np.array([surf_mbb.corner_points[2][0], surf_mbb.corner_points[2][1], np.array(surf[3])[2]])

    else:
        surf_cor2d = surf_cor[:, 0:2]
        surf_mbb = mbr(surf_cor2d)
        surf_mbb_rm = np.array([[np.cos(surf_mbb.unit_vector_angle), -np.sin(surf_mbb.unit_vector_angle), 0],
                                [np.sin(surf_mbb.unit_vector_angle), np.cos(surf_mbb.unit_vector_angle), 0],
                                [0, 0, 1]])
        affine_m = surf_mbb_rm
        center = np.array([surf_mbb.corner_points[2][0], surf_mbb.corner_points[2][1], surf_cor[:, 2].mean()])

    last_row = np.zeros(4)
    last_row[-1] = 1
    length = surf_mbb.length_parallel
    width = surf_mbb.length_orthogonal
    height = center[2] - surf[-1]
...
def add_mesh_geon(id, top_surf, bottom_surf, offset):
    ...
    return dict(type='mesh', id='mesh_' + str(id), transform=dict(affine_matrix=np.identity(4).tolist()),
             vertices_3d=point_cor.tolist(), faces=wall_index)
...
def add_shed_geon(id, surf, body_Z, offset):
    ...
    return dict(type='shed', id='shed_' + str(id), transform=dict(affine_matrix=affine_m.tolist()),
                width=roof_width, length=roof_length, height=height, theta=theta)
...
def add_gable_geon(id, surfs, body_Z, offset):
    ...
    return dict(type='gable', id='gable_' + str(id), transform=dict(affine_matrix=affine_m.tolist()), width=roof_width,
                length=roof_length, roof_height=gabel_roof_height, body_height=body_height)