...
valpaths = {k: os.path.join(config.dataset_path, v).replace('DaytonJacksonville', '4AOIs') for k,v in paths.items()}
...
def single_train(config):
    ...
    #onetrain(trainds, valds, ntrain, nval, config, num_workers=num_workers, transforms=augment_flips_color)
    ...
    onetrain(trainds, valds, ntrain, nval, config, num_workers=num_workers, transforms=augment_flips_color)
    #onetrain(trainds, valds, ntrain, nval, config, num_workers=num_workers, transforms=augment_flips_color, num_channels_changed=True)

    #3rd stage: change the loss function.
    config = update_config(config, loss=config.loss + '_w', nb_epoch=num_epochs + num_epochs)
    onetrain(trainds, valds, ntrain, nval, config, num_workers=num_workers, transforms=augment_flips_color)
