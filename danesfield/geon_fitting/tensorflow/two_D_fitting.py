...
def fit_2D_curve(n, points, fit_type='poly2', dist_threshold=0.05):
    ...
    ey = e2
    ez = np.cross(ex,ey)
    fitted_indices, coefficients, mean_diff = fit2Dshapes(points_2d, fit_type = fit_type,  dist_threshold=dist_threshold)
    if fit_type == 'ellipse':
...
def check_2D_curve(n, coefficients, centroid, points, fit_type='poly2', dist_threshold=0.05):
    ...
    # points_2d has been move to centroid and e1, e2 as axes
    points_2d, e1, e2 = project2plane(points, centroid, n)
    fitted_indices, error = check2Dshapes(points_2d, coefficients, fit_type = fit_type,  dist_threshold=dist_threshold)
...
def transform_ellipse(e1, e2, coefficients, n):
    ...
    R_matrix = [[cos_phi + nx**2*(1 - cos_phi), nx * ny * (1 - cos_phi) - nz * sin_phi, nx * nz * (1 - cos_phi) + ny * sin_phi],
                [nx * ny * (1 - cos_phi) + nz * sin_phi, cos_phi + ny**2*(1 - cos_phi), ny * nz * (1 - cos_phi) - nx * sin_phi],
...
def project2plane(points_3d, centroid, n, x=None):
    ...
    points_2d = np.concatenate([np.expand_dims(t_1, axis=1), np.expand_dims(t_2, axis=1)], axis = 1)
...
def fit2Dshapes(points_2d, fit_type="poly2", dist_threshold = 0.05):
    ...
    if fit_type == "ellipse":
        ...
        def elip(X, P):
            P_cond = (P[0] - ellipse_center[0]) ** 2 / ellipse_width ** 2 + (P[1] - ellipse_center[1]) ** 2 / ellipse_height ** 2 - 1
            X_cond = (X[0] - ellipse_center[0]) ** 2 / ellipse_width ** 2 + (X[1] - ellipse_center[1]) ** 2 / ellipse_height ** 2 - 1
            if P_cond >= 0:
                return -1 * X_cond
            return X_cond
        residuals, fitted_indices = find_min_dist_residual(elip, points_2d, dist_threshold)
        return fitted_indices, [ellipse_center, ellipse_width, ellipse_height, ellipse_phi], residuals
    # poly curve
    if fit_type == "poly2":
        poly_coefficients, residuals, _ , _ , _ = np.polyfit(data[0], data[1], 2, full=True)
...
def find_min_dist_residual(f, points, dis_threshold):
    ...
    for i in range(points.shape[0]):
        ...
        X = fmin_cobyla(objective, x0=[P], cons=[c1], args=([P]), consargs=([P]))
        # print objective(X, P)
        dist = objective(X, P)
