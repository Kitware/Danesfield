...
def merge_onetrain_tiffs(root, test_dsmdata_dir):
    prob_files = {f for f in os.listdir(root) if f.split('.')[-1] in ['tif', 'tiff']}
    ...
    for prob_file in tqdm.tqdm(prob_files):
        dsm = gdal.Open(os.path.join(test_dsmdata_dir, prob_file.replace('onetrain', '')), gdal.GA_ReadOnly)
        ...
        res_path_geo = os.path.join(root, 'merged', prob_file.replace('.tif', '_Prob.tif').replace('onetrain', ''))
