...
class GdalSaver(Evaluator):
    ...
    def save(self, name, prefix=""):
        ...
        res_path_geo = os.path.join(
            self.config.results_dir, 'results', self.config.folder, prefix + name)
        driver = gdal.GetDriverByName('GTiff')
        outRaster = driver.Create(
            res_path_geo, self.full_pred.shape[1], self.full_pred.shape[0], 1, gdal.GDT_Float32)
        if os.path.isfile(ref_file):
            ...
            if has_mask:
                ...
                empty_mask = np.zeros(
                    (self.full_pred.shape[0], self.full_pred.shape[1]))
