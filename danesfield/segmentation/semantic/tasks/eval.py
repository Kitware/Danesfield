...
def predict(model, batch, flips=flip.FLIP_NONE):
    ...
    if flips > flip.FLIP_NONE:
        ...
        if flips > flip.FLIP_LR:
            ...
            pred4 = flip_tensor_ud(flip_tensor_lr(
...
def read_model(config, fold):
    ...
    with warnings.catch_warnings():
        ...
        model = torch.load(os.path.join(
            config.results_dir, 'weights', config.folder, 'fold{}_best.pth'.format(fold)))
        model.eval()
        return model
...
def read_onetrain_model(config):
    ...
    with warnings.catch_warnings():
        ...
        model_path = os.path.join(
            config.results_dir, 'weights', config.folder, 'onetrain_best.pth')

        model = None
        if '_checkpoint' in model_path:
            checkpoint = torch.load(os.path.join(
                config.results_dir, 'weights', config.folder, 'onetrain_checkpoint.pth'))
...
class Evaluator:
    """
    base class for inference, supports different strategy -
    full image or crops, also supports different image types
    """
    ...
    def visualize(self, show_light=False, show_base=True):
        ...
        if self.full_image is not None and show_light:
            light_heat = cv2.addWeighted(
                self.full_image[:, :, :3], 0.6, hmap, 0.4, 0)
            if dsize:
            ...
            if self.full_mask is not None and self.show_mask:
                light_mask = cv2.addWeighted(self.full_image[:, :, :3], 0.6, cv2.cvtColor(
                    self.full_mask, cv2.COLOR_GRAY2BGR), 0.4, 0)
                if dsize:
        ...
        if self.full_image is not None and show_base:
            ...
            if self.full_mask is not None and self.show_mask:
                if dsize:
                    cv2.imshow('mask', cv2.resize(
                        self.full_mask, (dsize, dsize)))
                else:
    ...
    def predict(self, skip_folds=None):
        for fold, (train_index, val_index) in enumerate(self.folds):
            ...
            val_dataset = ds_cls(self.ds, val_index,
                                 stage='test', config=self.config)
            val_dl = PytorchDataLoader(
                val_dataset, batch_size=self.config.predict_batch_size,
                num_workers=self.num_workers, drop_last=False)
            model = read_model(self.config, fold)
            pbar = val_dl if self.config.dbg else tqdm.tqdm(
                val_dl, total=len(val_dl))
            for data in pbar:
    ...
    def on_image_constructed(self, prefix=""):
        ...
        if self.full_mask is not None:
            ...
            if np.any(self.full_pred > .5) or np.any(self.full_mask >= 1):
                d = 1 - dice(self.full_pred.flatten() > .5,
                             self.full_mask.flatten() >= 1)
                self.dice.append(d)
                if self.config.dbg:
        ...
        for i in range(3):
            for j in range(3):
                samples = torch.autograd.Variable(
                    data['image'][:, :, insidx[i]:ineidx[i], insidx[j]:ineidx[j]],
                    volatile=True).cuda()
...
#        print('youtedix: {}'.format(youteidx))
        ...
        for i in range(len(xinsidx)):
            for j in range(len(yinsidx)):
                samples = torch.autograd.Variable(
                    data['image'][:, :, xinsidx[i]:xineidx[i], yinsidx[j]:yineidx[j]],
                    volatile=True).cuda()
                prediction = predict(model, samples, flips=self.flips)
