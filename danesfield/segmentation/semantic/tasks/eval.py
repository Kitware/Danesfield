...
def read_onetrain_model(config):
    ...
    with warnings.catch_warnings():
        warnings.simplefilter('ignore', SourceChangeWarning)
        model_path = os.path.join(config.results_dir, 'weights', config.folder, 'onetrain_best.pth')
        
        model = None
        if '_checkpoint' in model_path:
            checkpoint = torch.load(os.path.join(config.results_dir, 'weights', config.folder, 'onetrain_checkpoint.pth'))
...
class Evaluator:
    ...
    def predict_samples_2048x2048(self, model, data):
        ...
        for i in range(3):
            for j in range(3):
                samples = torch.autograd.Variable(data['image'][:,:,insidx[i]:ineidx[i], insidx[j]:ineidx[j]], volatile=True).cuda()
                prediction = predict(model, samples, flips=self.flips)
                predicted[:, :, outsidx[i]:outeidx[i], outsidx[j]:outeidx[j]] = prediction[:,:,cpsidx[i]:cpeidx[i], cpsidx[j]:cpeidx[j]]
                #predicted[:,0:1, (i*width):((i+1)*width), (j*width):((j+1)*width)] = predict(model, samples, flips=self.flips)
...
#        print('youtedix: {}'.format(youteidx))
        ...
        for i in range(len(xinsidx)):
            for j in range(len(yinsidx)):
                samples = torch.autograd.Variable(data['image'][:,:,xinsidx[i]:xineidx[i], yinsidx[j]:yineidx[j]], volatile=True).cuda()
                prediction = predict(model, samples, flips=self.flips)
