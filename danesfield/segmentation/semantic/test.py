...
def predict():
    ...
    ds = ImageProvider(MultibandImageType, paths,
                       border=0, image_suffix='.png')

    print('ds: {}'.format(ds))

    folds = [([], list(range(len(ds)))) for i in range(5)]
    num_workers = 0 if os.name == 'nt' else 8
    keval = GdalFullEvaluator(
        config, ds, folds, test=True, num_workers=num_workers, border=0)
    keval.onepredict()


if __name__ == "__main__":
    config = update_config(config, img_rows=2048, img_cols=2048,
                           target_rows=2048, target_cols=2048, num_channels=5)
