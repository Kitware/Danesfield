...
# Wrapper function to run the segmentation code
def get_pred(img_data, depth_data, NDVI_data, num_category):
    if args.no_NDVI:
    ...
    pred = pred.reshape(
            int(img_data.shape[0]/training_stride),
            int(img_data.shape[1]/training_stride))
    return pred
...
test_img_data, test_dhm, test_NDVI, dsm = read_data_test()
# resize image
...
pred = get_pred(test_img_data, test_dhm_data, test_NDVI_data, num_valid_category)
