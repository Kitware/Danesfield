...
with no input imagery, just a point cloud.
'''
...
def create_working_dir(working_dir=None):
    '''
    Create a working directory for running algorithms.
    All files generated by the system are written to this directory.

    :param working_dir: directory to create for work files;
    if None or empty, create using date/time
